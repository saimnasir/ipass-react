...
import { EnvironmentTypeModelEnhancedTableProps } from '../../library/props/EnhancedTable'
import { createEnvironmentType, getEnvironmentType, getEnvironmentTypes, updateEnvironmentType } from '../../network/services/environmentTypeService'
import { Edit } from '@mui/icons-material'
import { blue } from '@mui/material/colors'
import EnvironmentTypeEditor from './EnvironmentTypeEditor'
import { SubmitHandler } from 'react-hook-form'
import { SingleResponse } from '../../models/single-response'
import AddCircleOutlineIcon from '@mui/icons-material/AddCircleOutline';
import EnvironmentTypeModelInit, { EnvironmentTypeModel } from '../../models/environment-type/environmentTypeModel'
import { Action } from '../../models/enums/Actions'

const dataCells: readonly DataCell<EnvironmentTypeModel>[] = [
  {
    id: 'title',
    numeric: false,
    disablePadding: true,
...
function EnhancedTableHead(props: EnvironmentTypeModelEnhancedTableProps) {
  const { onSelectAllClick, order, orderBy, numSelected, rowCount, onRequestSort, onOpenEditor } =
    props;
  const createSortHandler =
    (property: keyof EnvironmentTypeModel) => (event: React.MouseEvent<unknown>) => {
...
const EnvironmentTypeList = () => {
  ...
  const [rows, setRows] = useState<EnvironmentTypeModel[]>([])
  const [totalItemCount, setTotalItemCount] = useState<number>(0)
  const [selected, setSelected] = React.useState<readonly string[]>([]);
  const [pagination, setPagination] = useState<PaginationFilterModel>(PaginationFilterModelInit)
  const [dense, setDense] = React.useState(false);
  const [environmentTypeModel, setEnvironmentTypeModel] = useState<EnvironmentTypeModel>(EnvironmentTypeModelInit)
  ...
  const loadTable = () => {
    getEnvironmentTypes(pagination)
  ...
  const handleRequestSort = (
    ...
    property: keyof EnvironmentTypeModel,
  ) => {
  ...
  const getCellValue = (rowData: EnvironmentTypeModel, cell: DataCell<EnvironmentTypeModel>, value: string | boolean | undefined) => {
  ...
  useEffect(() => {
    if (environmentTypeModel.id) {
      setLoading(true)
      getEnvironmentType(environmentTypeModel.id)
        .then((response: AxiosResponse<FinalResponse<SingleResponse<EnvironmentTypeModel>>>) => {
          setApiResponse(response.data)
          setEnvironmentTypeModel(response.data.data.data)
        })
        .catch(err => {
          console.log('error', err)
        }).finally(() => {
  ...
  const onUpdate = (formData: EnvironmentTypeModel) => {
    updateEnvironmentType(formData)
  ...
  const onCreate = (formData: EnvironmentTypeModel) => {
    createEnvironmentType(formData)
      .then((response: AxiosResponse<FinalResponse<boolean>>) => {
        setApiResponse(response.data)
        if (response.data.success) {
          setTimeout(() => {
  ...
  const onSubmitHandler: SubmitHandler<EnvironmentTypeModel> = (formData) => {
    setLoading(true)
    setEnvironmentTypeModel(formData)
  ...
  const handleOpenEdit = (id: string) => {
    ...
    setEnvironmentTypeModel({ ...EnvironmentTypeModelInit, id: id })
  }
...
export default EnvironmentTypeList